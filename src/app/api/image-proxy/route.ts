...
async function fetchImage(url: string, referer?: string): Promise<NextResponse> {
  await initProxy();
...
export async function GET(request: NextRequest) {
  try {
    ...
    const params = parseQueryData(dataParam);

    if (!params || !params.url) {
      return NextResponse.json({ error: 'Invalid or missing data parameter' }, { status: 400 });
    }

    return await fetchImage(params.url, params.referer);
  } catch (error) {
