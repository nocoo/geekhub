...
现在由 `reader-view-model.ts` 管理查询、状态和写入，视图负责布局、焦点与滚动。每次进入视图保留独立的阅读上下文：阅读状态就地更新，后台发现新文章后自动刷新列表，按发布时间显示新内容，并用 toast 告知新增篇数。列表持续记录首个仍存在的可见文章及其偏移，刷新和标题自动翻译时恢复这个位置；手机阅读正文期间也保留返回列表的锚点。标题和简介翻译静默更新列表，不改动正在阅读的正文或标题。正文由独立的 Markdown 组件呈现；阅读状态和元数据变化复用正文 DOM，保留选区。后台请求失败会保留已经加载的内容。
...
每个列表阅读快照有独立标识。已读数、同步状态等元数据变化不刷新文章；订阅内容版本变化时自动载入，回到过期的历史快照也自动更新。刷新失败保留已有内容，并提供重试入口；同一失败版本不会不断自动重试。分页请求完成前发生新导航，即使随后回到同一篇文章，也不能由旧请求擅自切换文章。删除当前源或分类替换当前历史项，避免多出一个已经失效的中间范围。
...
| 条目完整度 | 20% | 标题与安全链接占 20%，有效正文／摘要占 60%，有效日期占 20%；正文和结构样本最多 200 条，日期覆盖全部条目 |
| 响应速度 | 10% | 成功读取 RSS 的耗时：≤1／3／5／10 秒为 100／85／60／30，更慢为 10；失败请求不计为快速响应 |
| 主站可达性 | 10% | 最终可达 HTTPS 为 100，仅 HTTP 可达为 60，均失败为 0；未知主站不计分 |

有效正文／摘要需清理 HTML、链接文本、重复标题及空白后至少 40 字符。仅标题、链接、图片或过短文本不算有效正文；该检查不保证提供了全文，也不评价文章质量。正文覆盖率写入诊断报告的 `contentEntries`，并在页面显示实际样本数量。

RSS 不可用总分最高 39，没有可导入条目或完全没有有效正文最高 49，超过停更阈值最高 59，不足半数条目有有效正文最高 69。旧报告没有正文检查数据时，条目完整度显示未知，暂定总分最高 79，提示重新检查。临时失败提示复查；404／410、疑似停更会建议寻找新地址或确认后暂停，保留已有文章。候选必须至少半数样本有正文、日期已知、未过时、最终地址不同，才会在当前源失效、缺少内容或过旧时获推荐，优先最近更新的地址，并要求用户核对内容属于同一订阅后手动替换。不会因新地址存在就建议更换健康订阅。

2026-09-19 实测 Hugging Face：HTTP 200，返回 862 条，前 200 条均只有元数据，有效正文／摘要为 0，综合评分 49。通过本地正常 TLS 的 Chrome 验证，诊断报告写入开发 D1：[实际报告](evidence/diagnostic-content/real-huggingface.json)、[截图](evidence/diagnostic-content/real-huggingface.png)。真实响应前三条另存为自动化测试夹具，避免测试依赖外部网络。
...
任务使用唯一令牌和递增版本。换源、暂停、删除或新任务接管后，旧任务不能继续写文章、覆盖校验头或修改新任务状态。重复投递已完成的任务不再联网；失败由 Queue 有限重试，后续通过手动刷新重试；不再配置 Cron 或自动刷新周期。

迁移前没有令牌的旧消息直接结束，租约到期后可通过手动刷新重新调度。文章仍以来源 ID 去重，不因重复抓取清空阅读状态。
